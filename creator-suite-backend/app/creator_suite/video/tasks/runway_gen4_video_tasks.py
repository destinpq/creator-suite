...
from app.creator_suite.video.providers.runway_gen4_video_provider import RunwayGen3AlphaVideoProvider
...
@celery_app.task(bind=True, base=CallbackTask, name="generate_runway_gen4_video",
                 soft_time_limit=600, time_limit=720)  # 10 minutes soft, 12 minutes hard limit
def generate_runway_gen4_video(self, task_id: str, input_data: Dict[str, Any]):
    """
    Celery task to generate video using Runway Gen-3 Alpha model.
    ...
    try:
        ...
        provider = RunwayGen3AlphaVideoProvider()
...
@celery_app.task(bind=True, name="cancel_runway_gen4_video_generation")
def cancel_runway_gen4_video_generation(self, task_id: str):
    """
    Cancel an ongoing Runway Gen-3 Alpha video generation task.
    ...
    try:
        ...
        # For Runway, we don't have a direct cancellation mechanism
