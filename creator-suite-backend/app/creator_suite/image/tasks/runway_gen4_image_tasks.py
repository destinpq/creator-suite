...
from app.creator_suite.image.providers.runway_gen4_image_provider import RunwayGen4ImageProvider
...
@celery_app.task(bind=True, base=CallbackTask, name="generate_runway_gen4_image",
                 soft_time_limit=300, time_limit=360)  # 5 minutes soft, 6 minutes hard limit
def generate_runway_gen4_image(self, task_id: str, input_data: Dict[str, Any]):
    """
    Celery task to generate image using Runway Gen-4 model.
    ...
    try:
        ...
        provider = RunwayGen4ImageProvider()
...
@celery_app.task(bind=True, name="cancel_runway_gen4_generation")
def cancel_runway_gen4_generation(self, task_id: str):
    """
    Cancel an ongoing Runway Gen-4 generation task.
    ...
    try:
        ...
        # For Runway, we don't have a direct cancellation mechanism like Replicate
