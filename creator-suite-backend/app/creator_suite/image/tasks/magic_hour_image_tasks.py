...
from app.creator_suite.image.providers.magic_hour_image_provider import MagicHourImageProvider
...
@celery_app.task(bind=True, base=CallbackTask, name="generate_magic_hour_image",
                 soft_time_limit=300, time_limit=360)  # 5 minutes soft, 6 minutes hard limit
def generate_magic_hour_image(self, task_id: str, input_data: Dict[str, Any]):
    """
    Celery task to generate image using Magic Hour model.
    ...
    try:
        ...
        provider = MagicHourImageProvider()
...
@celery_app.task(bind=True, name="cancel_magic_hour_generation")
def cancel_magic_hour_generation(self, task_id: str):
    """
    Cancel an ongoing Magic Hour generation task.
